...
class Utils {
  ...
  // Function for Getting the Frequency of an element in an array
  getElemFreq = (arr,elem) => {
...
export class Stock_Market_Dynamics_Emulator {
  constructor(size_tuple, p_d, p_h, p_c,k,l) {
    ...
    // Filling the Present Config as a Grid of 0s
    this.config = [];
    ...
    // Getting the Initial Random Configuration
    this.getRandominitialConfig();
  }

  // <= 20% will be active traders
    ...
    while (1) {
      ...
      for (let i = 0; i < this.config.length; i++) {
        for (let j = 0; j < this.config[0].length; j++) {
          ...
          // Applying the Rule of the present cell being 1. Here we check for at least "k" neighbours instead of at least 1 
          if (this.config[i][j] == 1) {
            if (this.UtilsObject.getElemFreq(neighbours,0) >= this.k)
              newConfig[i][j] = Math.random() <= this.p_d ? 0 : newConfig[i][j];
          }

          // Applying the Rule of the present cell being 0. Here we check for at least "l" neighbours instead of at least 1 
          if (this.config[i][j] == 0) {
            if (this.UtilsObject.getElemFreq(neighbours,1) >= this.l) {
