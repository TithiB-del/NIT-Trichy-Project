...
export class Stock_Market_Dynamics_Emulator {
  ...
  FindNeighbours(i, j) {
    ...
    // Choosing Two Different Local Neighbours at Random
    let idx = this.UtilsObject.getRandomNum(0,3);
    let idx2 = this.UtilsObject.getRandomNum(0,3);
    while(idx == idx2){
      idx2 = this.UtilsObject.getRandomNum(0,3);
    }
    masterNeighbourArray.push(neighbours[idx]);
    masterNeighbourArray.push(neighbours[idx2]);


    // Choosing Two Global Neighbours at Random
    let idx_row = this.UtilsObject.getRandomNum(0,this.size_tuple[0]-1);
    let idx_col = this.UtilsObject.getRandomNum(0,this.size_tuple[1]-1);
    let idx2_row = this.UtilsObject.getRandomNum(0,this.size_tuple[0]-1);
    let idx2_col = this.UtilsObject.getRandomNum(0,this.size_tuple[1]-1);
    while(idx2_row == idx_row && idx2_col == idx_col){
      idx2_row = this.UtilsObject.getRandomNum(0,this.size_tuple[0]-1);
      idx2_col = this.UtilsObject.getRandomNum(0,this.size_tuple[1]-1);
    }
    masterNeighbourArray.push(this.config[idx_row][idx_col]);
    masterNeighbourArray.push(this.config[idx2_row][idx2_col]);



    return masterNeighbourArray;
  }
